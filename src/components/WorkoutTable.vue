...
import { WORKOUT_COLUMNS, getWorkoutRowColor } from '../stores/workouts.js'
import { getDateColor } from '../stores/habits.js'
...
const columns = WORKOUT_COLUMNS
const nonDateColumns = WORKOUT_COLUMNS.filter(c => c.key !== 'date')
const ROW_HEIGHT = 36
const POOL_SIZE = 50
...
function formatValue(value, format, colType) {
  if (value === null || value === undefined || value === '') return ''
  if (colType === 'text') return String(value)
  const n = Number(value)
  if (isNaN(n)) return String(value)
  if (format === 'kg') return `${n.toFixed(1)} kg`
...
function updateVisibleRows() {
  ...
  for (let i = 0; i < POOL_SIZE; i++) {
    ...
    // Row color based on workout type
    const rc = getWorkoutRowColor(row)
    ...
    for (let j = 0; j < nonDateColumns.length; j++) {
      ...
      // Apply row color to all non-date cells
      td.style.backgroundColor = rc ? rc.bg : ''
      td.style.color = rc ? rc.text : ''
